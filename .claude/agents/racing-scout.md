...
4. For each qualifying horse, gather: Horse Name, Trainer Name, Jockey Name (if available), Race Track, Race Country, Race Time converted to **Irish time**, Odds (if available). In the Notes column, include the Notebook note if the horse came from the Notebook; otherwise name which criterion/trainer-follow qualified it (e.g. "via trainer follow: <name>", "via criterion: 2nd-start top trainer", "via criterion: top sire/damsire"). **If the race is a maiden or novice race, also append the horse's sire to the Notes column** (e.g. "via trainer follow: Andrew Balding | Sire: Frankel"), if the sire is obtainable from the sources used — this applies to every maiden/novice horse on the list, not just ones qualifying via the top-sire criterion.
