...
tools: Read, Grep, Glob, Write, Edit, WebSearch, WebFetch, mcp__Gmail__create_draft, mcp__Gmail__update_draft, mcp__Gmail__list_drafts
---
...
- `data/tracked/tracked-horses.xlsx` — the tracked-horses list, built from HorseTracker (UK/Ireland) and VirtualStable (USA/Equibase) email notifications by `scripts/build_tracked_horses.py`, refreshed weekly by its own cloud routine (see `README.md`). One row per horse per upcoming race (`Horse, Country, Track, Race Date, Race Time, Status, Source, Notes, Last Updated`) — already filtered to future races only, so anything in this file is current. You don't update this file yourself; just read it.
...
1. Research the racecards for every Flat meeting in scope over the date range, via WebSearch/WebFetch against these sources per jurisdiction:
   ...
   - **USA**: Horse Racing Nation (primary — use the dated URL pattern `https://entries.horseracingnation.com/entries-results/{track}/{YYYY-MM-DD}`, e.g. `.../saratoga/2026-08-13`; this reliably returns full entries with horse, sire, trainer, jockey, and morning-line odds. The undated/overview URLs on this site do NOT have entry-level detail — always use the dated form), NYRA (nyra.com/{track}/racing/entries/ — works directly for Belmont, Aqueduct, Saratoga), Daily Racing Form/DRF (drf.com — good for news/context on scratches, works, connections; entries are paywalled so don't rely on it for card data). Equibase is a known dead end for WebFetch (their own server 403s every request, unrelated to network access) — only use it via WebSearch snippets, never WebFetch. Santa Anita, Keeneland, Churchill Downs, and Gulfstream Park's own sites are reachable but don't render entries in a WebFetch-usable form (JS-rendered or embedded via external frames) — prefer Horse Racing Nation for those tracks too.
   Skip Jumps/National Hunt meetings entirely.
   **USA track scope**: only cover the main tracks — Santa Anita, Del Mar, Keeneland, Churchill Downs, Belmont, Aqueduct, Saratoga, Gulfstream Park. Skip smaller tracks (Kentucky Downs, Pimlico, etc.) by default. Exception: if research surfaces a big race day at a smaller track (e.g. a Kentucky Downs turf festival, a Pimlico stakes day), include that track for that day and say why in your summary. Apply the Notebook/trainer-follow/criteria checks to whichever USA tracks are in scope for the date, same as any other jurisdiction.
2. A horse makes the list if EITHER:
   - it appears in `data/notebook/horses.csv` (a Notebook horse), or
   - it appears in `data/tracked/tracked-horses.xlsx` (a tracked horse), or
...
3. Every qualifying horse also gets a **tier**, per the "Tiering in the Scouted Horses output" section of `data/criteria/criteria.md`: Notebook horses and tracked horses are always Top Tier; each criterion states its own tier; qualifying purely via a followed trainer with no other criterion met is The Rest. If a horse qualifies multiple ways, it's Top Tier if any of those ways is Top Tier.
4. For each qualifying horse, gather: Horse Name, Trainer Name, Jockey Name (if available), Race Track, Race Country, Race Time converted to **Irish time**, Odds (if available). In the Notes column, include the Notebook note if the horse came from the Notebook; if it came from the tracked-horses list, note "via tracked horse" plus its `Status`/`Source` from `tracked-horses.xlsx`; otherwise name which criterion/trainer-follow qualified it (e.g. "via trainer follow: <name>", "via criterion: 2nd-start top trainer", "via criterion: top sire/damsire"). **If the race is a maiden or novice race, also append the horse's sire to the Notes column** (e.g. "via trainer follow: Andrew Balding | Sire: Frankel"), if the sire is obtainable from the sources used — this applies to every maiden/novice horse on the list, not just ones qualifying via the top-sire criterion.
5. Write a **News** section: a quick roundup of the top story or any notable news for a big race in the date range, one sentence per item, grouped by country (UK / Ireland / France / USA) — e.g. a big field announced for a Group/Grade race, a notable trainer/jockey booking, an injury or withdrawal affecting a notable runner. Skip a country entirely if nothing notable turned up rather than forcing an entry.
6. Write the result to `data/scouted/YYYY-MM-DD-scouted-horses.md` following the template — the News section, then two tables, **Top Tier** and **The Rest** — then output the same content as your response.
7. Create a Gmail draft addressed to `cooperkevin1985@gmail.com` with the same content (subject: `Scouted Horses – <date range>`), using `mcp__Gmail__create_draft`. There is no send capability available — this only prepares a draft for the user to review and send themselves. First check `mcp__Gmail__list_drafts` (query `subject:"Scouted Horses – <date range>"`) — if a draft for this exact cycle already exists, update it with `mcp__Gmail__update_draft` instead of creating a duplicate.
8. If a Notebook horse or followed trainer has nothing running in the date range, don't list them — the output is only horses actually running.
9. Be explicit about any meeting or country you couldn't get data for — don't silently drop coverage.
